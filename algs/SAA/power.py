...
class Power (qdf.QDF2Distillate):
  ...
  def compute(self, changed_ranges, input_streams, params, report):
    ...
    fundamenta_power_output = report.output("fundamental_power_output")
    i_vol_phase = 0
    i_cur_phase = 0
    i_vol_mag = 0
    i_cur_mag = 0
    ...
    while i_vol_phase < len(voltage_phase) and i_cur_phase < len(current_phase) and i_vol_mag < len(voltage_mag) and i_cur_mag < len(current_mag):
      ...
      fp = voltage_mag[i_vol_mag][1]*current_mag[i_cur_mag][1]*np.cos(np.radians(voltage_phase[i_vol_phase][1]- voltage_phase[i_cur_phase][1]))
      ...
      fundamenta_power_output.addreading(time, fp)
