...
class sequence (qdf.QDF2Distillate):
  ...
  def compute(self, changed_ranges, input_streams, params, report):
    ...
    while idxM1<len(M1) and idxA1< len(A1) and idxM2< len(M2) and idxA2< len(A2) and idxM3 < len(M3) and idxA3 < len(A3):
      ...
      sinA2=np.sin(np.radians(A2[idxA2][1]-A1[idxA1][1]))
      sinA3_add120=np.sin(np.radians(A3[idxA3][1]+120-A1[idxA1][1]))
      sinA3_add240=np.sin(np.radians(A3[idxA3][1]+240-A1[idxA1][1]))
      sinA2_add120=np.sin(np.radians(A2[idxA2][1]+120-A1[idxA1][1]))
      sinA2_add240=np.sin(np.radians(A2[idxA2][1]+240-A1[idxA1][1]))
      # compute cosin value for three phase and cosin value for l2 and l3 anfter add 120 degree and 240 degree
      cosA3=np.cos(np.radians(A3[idxA3][1]-A1[idxA1][1]))
      cosA1=np.cos(np.radians(A1[idxA1][1]-A1[idxA1][1]))
      cosA2=np.cos(np.radians(A2[idxA2][1]-A1[idxA1][1]))
      cosA3_add120=np.cos(np.radians(A3[idxA3][1]+120-A1[idxA1][1]))
      cosA3_add240=np.cos(np.radians(A3[idxA3][1]+240-A1[idxA1][1]))
